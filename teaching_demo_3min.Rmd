...
dsm_harv <- raster('https://github.com/oharac/geospatial/raw/main/data/dsm_harv_crop.tif')
# DTM data for Harvard Forest
dtm_harv <- raster('https://github.com/oharac/geospatial/raw/main/data/dtm_harv_crop.tif')
```

Examine the attributes of the two rasters
```{r}
dsm_harv; dtm_harv
```

```{r}
plot(dsm_harv, main = 'dsm'); plot(dtm_harv, main = 'dtm')
```
...
To plot, convert the raster to a data frame:
``` {r}
